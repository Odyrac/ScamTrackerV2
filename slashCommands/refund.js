...
module.exports = {
    name: "refund", //the command name for the Slash Command
    description: "Permet d'ajouter un joueur comme ayant remboursé", //the command description for Slash Command Overview
    cooldown: 5,
    ...
    run: async (client, interaction, message) => {
        try {
            ...
            const reason = 'Ce joueur a remboursé le stuff.'
            ...
            .setTitle(`\`${pseudo2}\` a remboursé son stuff !`)
            .setThumbnail(`https://crafatar.com/renders/body/${uuid}?overlay`)
            .setColor("#35f009")
            .setDescription(`Il n'est dorénavant plus présent comme scammer dans notre base de données. Néanmoins, il reste détectable avec la commande \`/check ${pseudo2}\`.\n\n__**Ajouté par :**__ ${author_tag}`)
            .setFooter({text: ee.footertext, iconURL: ee.footericon})

            
            
            const logembed = new MessageEmbed()
            .setTitle(`Un joueur a remboursé le stuff volé !`)
            .setColor('#ee942e')
            ...
            if (currentlyBL.length) {
              ...
              await new asSettings({
                uuid: uuid,
                gid: interaction.guildId,
                current: true,
                author: interaction.user.id,
                authora: results.author,
                timestamp: new Date().getTime(),
                timestampa: results.timestamp,
                reason,
                reasona: results.reason
              ...
              if (currentlyas.length) {
                ...
                .setColor("#ff0000")
                .setDescription(`Ce joueur a déjà été désigné comme ayant remboursé son stuff !`)
                return interaction.reply({ embeds: [embederror] })
              } else {
                const embederror = new MessageEmbed()
                .setTitle(`Joueur non-présent !`)
                .setColor("#ff0000")
                .setDescription(`Ce joueur n'est pas dans la ScamList !`)
                return interaction.reply({ embeds: [embederror] })
              }
            }
